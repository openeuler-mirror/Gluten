...
object ConventionFunc {
  ...
  private class BuiltinFunc(o: Override) extends ConventionFunc {
    ...
    private def conventionReqOf0(plan: SparkPlan): Seq[ConventionReq] = plan match {
      ...
      case write: DataWritingCommandExec if GlutenConfig.get.enableNativeWriter.getOrElse(false) &&
          SparkShimLoader.getSparkShims.isPlannedV1Write(write) => // TODO: promotion is avoid c2r
