...
PlanNodePtr SubstraitToOmniPlanConverter::ToOmniPlan(
    const ::substrait::GenerateRel &generateRel) {}
...
PlanNodePtr
SubstraitToOmniPlanConverter::ToOmniPlan(const ::substrait::SetRel &setRel) {
  ...
  default:
    OMNI_THROW("Substrait Error",
               "Unsupported SetRel op: " + std::to_string(setRel.op()));
  }
}

PlanNodePtr
SubstraitToOmniPlanConverter::ToOmniPlan(const ::substrait::JoinRel &joinRel) {}
...
PlanNodePtr SubstraitToOmniPlanConverter::ToOmniPlan(
  ...
  return std::make_shared<LimitNode>(
      NextPlanNodeId(), static_cast<int32_t>(fetchRel.offset()),
      static_cast<int32_t>(fetchRel.count()), false, childNode);
}

PlanNodePtr
SubstraitToOmniPlanConverter::ToOmniPlan(const ::substrait::TopNRel &topNRel) {
  ...
  return std::make_shared<TopNNode>(
      NextPlanNodeId(), sortingKeys, sortingOrders, sortNullFirsts,
      static_cast<int32_t>(topNRel.n()), childNode);
}

PlanNodePtr
SubstraitToOmniPlanConverter::ToOmniPlan(const ::substrait::ReadRel &readRel,
                                         const DataTypesPtr &type) {}
...
PlanNodePtr SubstraitToOmniPlanConverter::ConstructValueStreamNode(
  ...
  if (!validationMode) {
    OMNI_CHECK(streamIdx <= inputIters.size(),
               "Could not find stream index {} in input iterator list.");
...
PlanNodePtr
SubstraitToOmniPlanConverter::ToOmniPlan(const ::substrait::SortRel &sortRel) {
  ...
  return std::make_shared<OrderByNode>(
      NextPlanNodeId(), sortingKeys, sortingOrders, sortNullFirsts, childNode);
}

int32_t SubstraitToOmniPlanConverter::GetStreamIndex(
...
PlanNodePtr
SubstraitToOmniPlanConverter::ToOmniPlan(const ::substrait::Rel &rel) {
...
PlanNodePtr
SubstraitToOmniPlanConverter::ToOmniPlan(const ::substrait::RelRoot &root) {
