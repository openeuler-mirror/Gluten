...
bool IsNullOnFailure(
  ...
  default:
    OMNI_THROW(
        "SUBSTRAIT_ERROR:", "The given failure behavior is NOT supported: '{}'",
...
TypedExprPtr SubstraitOmniExprConverter::ToOmniExpr(
  ...
  default:
    OMNI_THROW("SUBSTRAIT_ERROR:",
               "Substrait conversion not supported for Reference '{}'",
               std::to_string(typeCase));
  }
}
...
TypedExprPtr SubstraitOmniExprConverter::ToOmniExpr(
  ...
  if (retTypeId == argReturnType->GetId()) {
    if (TypeUtil::IsStringType(argReturnType->GetId())) {
      ...
      auto argScale =
          static_cast<DecimalDataType *>(argReturnType.get())->GetScale();
      auto argPrecision =
          static_cast<DecimalDataType *>(argReturnType.get())->GetPrecision();
      auto retScale = static_cast<DecimalDataType *>(retType.get())->GetScale();
      auto retPrecision =
          static_cast<DecimalDataType *>(retType.get())->GetPrecision();
...
TypedExprPtr SubstraitOmniExprConverter::ToOmniExpr(
  ...
  case ::substrait::Expression_Literal::LiteralTypeCase::kFixedChar: {
    ...
    return new LiteralExpr(stringVal,
                           CharType(substraitLit.fixed_char().length()));
  }
  case ::substrait::Expression_Literal::LiteralTypeCase::kDecimal: {
    ...
    if (precision <= DECIMAL64_DEFAULT_PRECISION) {
      ...
      auto *dec128String = new std::string(decimal);
...
TypedExprPtr SubstraitOmniExprConverter::ToOmniExpr(
  ...
  if (TypeUtil::IsStringType(falseExpr->GetReturnTypeId()) &&
    ...
    delete falseExpr;
    auto literalExpr =
        ParserHelper::GetDefaultValueForType(trueExpr->GetReturnTypeId());
...
std::shared_ptr<const LiteralExpr>
SubstraitOmniExprConverter::literalsToConstantExpr(
