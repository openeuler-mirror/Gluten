...
object OmniAdaptorUtil {
    ...
    val vec: Vec = datatype match {
      ...
      case StringType =>
        ...
        for (i <- 0 until columnSize) {
          if (null != columnVector.getUTF8String(i)) {
            System.arraycopy(
              columnVector.getUTF8String(i).getBytes,
              0,
              values,
              offsets(i),
  ...
  def addAllAndGetIterator(
      operator: OmniOperator,
      inputIter: Iterator[ColumnarBatch],
      schema: StructType,
      addInputTime: SQLMetric,
      numInputVecBatches: SQLMetric,
      numInputRows: SQLMetric,
      getOutputTime: SQLMetric,
      numOutputVecBatches: SQLMetric,
    ...
    new Iterator[ColumnarBatch] {
      ...
      override def next(): ColumnarBatch = {
        ...
        getOutputTime += NANOSECONDS.toMillis(System.nanoTime() - startGetOp)
        val vectors: Seq[OmniColumnVector] =
          OmniColumnVector.allocateColumns(vecBatch.getRowCount, schema, false)
