...
import org.apache.gluten.vectorized.OmniColumnarRow
...
/**
 * Interprets Spark expressions row by row and writes the results directly into Omni column vectors.
 */
class InterpretedOmniProjection(expressions: Seq[Expression]) extends OmniProjection {
  ...
  private[this] var mutableRow: OmniColumnarRow = null

  override def target(row: OmniColumnarRow): OmniProjection = {
    mutableRow = row
    this
  }

  private def getStringWriter(ordinal: Int, dt: DataType): (OmniColumnarRow, Any) => Unit =
    dt match {
  ...
  override def apply(input: InternalRow): OmniColumnarRow = {
...
object InterpretedOmniProjection {
  def createProjection(exprs: Seq[Expression]): OmniProjection =
    new InterpretedOmniProjection(exprs)
}
