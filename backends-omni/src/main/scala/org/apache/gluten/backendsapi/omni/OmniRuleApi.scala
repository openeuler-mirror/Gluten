...
import org.apache.gluten.extension.{OmniAggregateRepeatedExpressionRewriteRule, OmniHLLRewriteRule, RewriteAQEShuffleRead}
...
object OmniRuleApi {
  private def injectSpark(injector: SparkInjector): Unit = {
    ...
    injector.injectOptimizerRule(OmniAggregateRepeatedExpressionRewriteRule.apply)
...
//    injector.injectPostHocResolutionRule(ArrowConvertorRule.apply)
    injector.injectPostHocResolutionRule(_ => FallbackViewScan.MarkViewChildren())
...
//    injector.injectPreTransform(c => ArrowScanReplaceRule.apply(c.session))
    injector.injectPreTransform(c => FallbackViewScan.FallbackScans(c.glutenConf, c.session))
    ...
    injector.injectTransform(
      c =>
        DeltaOffloadRegistry.wrapTransformRule(
          HeuristicTransform.WithRewrites(validatorBuilder(c.glutenConf), rewrites, offloads)))

    // Legacy: Post-transform rules.
    DeltaOffloadRegistry.injectPostTransformRules(injector)
