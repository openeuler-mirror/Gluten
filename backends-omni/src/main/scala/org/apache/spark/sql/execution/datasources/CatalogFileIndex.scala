  ...
  def filterPartitions(filters: Seq[Expression]): InMemoryFileIndex = {
    if (table.partitionColumnNames.nonEmpty) {
      ...
      val partitions = selectedPartitions.map { p =>
        var path = new Path(p.location)
        val pathUri = path.toUri
        val scheme = Option(pathUri.getScheme)
        val authority = Option(pathUri.getAuthority)
        path = (path.isAbsolute, scheme, authority) match {
          case (true, Some(_), Some(_)) => path
          case _ =>
