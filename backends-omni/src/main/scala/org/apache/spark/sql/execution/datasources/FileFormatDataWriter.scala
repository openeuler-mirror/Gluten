...
class SingleDirectoryDataWriter(
  ...
  override def write(record: InternalRow): Unit = {
    ...
    try {
      ...
      record match {
        case fakeRow: FakeRow => fakeRow.batch.close()
        case _ =>
      }
    }
    statsTrackers.foreach(_.newRow(currentWriter.path, record))
    val curBatchRows = record match {
      case row: FakeRow => row.batch.numRows()
      case _ => 1
    }
    recordsInFile += curBatchRows
  }
}
...
class DynamicPartitionDataSingleWriter(
  ...
  override def write(record: InternalRow): Unit = {
    if (record.isInstanceOf[FakeRow]) {
      splitWrite(record)
    } else {
  ...
  private def splitWrite(fakeRow: InternalRow): Unit = {
    val batch = fakeRow.asInstanceOf[FakeRow].batch
    try {
      ...
      for (i <- 0 until numRows) {
        ...
        if (currentPartitionValues != nextPartitionValues || currentBucketId != nextBucketId) {
          ...
          fileCounter = 0
          if (i != 0) {
            writeRecord(fakeRow, lastIndex, i)
            lastIndex = i
          }
          renewCurrentWriter(currentPartitionValues, currentBucketId, closeCurrentWriter = true)
        } else if (
      ...
      if (lastIndex < batch.numRows()) {
        writeRecord(fakeRow, lastIndex, numRows)
      }
    } finally {
      if(batch != null) {
        batch.close()
