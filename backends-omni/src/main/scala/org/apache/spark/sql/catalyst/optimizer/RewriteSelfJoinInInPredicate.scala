 ...
 */
case class RewriteSelfJoinInInPredicate(spark: SparkSession)
    ...
    plan.transform {
      case f: Filter =>
        f.transformExpressions {
          case in @ InSubquery(
                _,
                listQuery @ ListQueryShim(
                  Project(projectList, Join(left, right, Inner, Some(joinCond), _)),
                  _,
                  _,
                  _,
                  _)) if left.canonicalized ne right.canonicalized =>
            ...
            val newProjectList = projectList.map {
              case attr: Attribute => attrMapping.getOrElse(attr, attr)
              case Alias(attr: Attribute, name) =>
                Alias(attrMapping.getOrElse(attr, attr), name)()
              case attr => attr
            }
              ...
              val filterPlan =
                if (nonEqualJoinAttrs.isEmpty) {
                  ...
                  Project(
                    newProjectList,
                    Filter(
                      buildBalancedPredicate(
                        nonEqualJoinAttrs.map(expr => GreaterThan(expr.toAttribute, Literal(0L))),
                        And),
                      aggPlan
                    )
                  )
  ...
  def apply(plan: LogicalPlan): LogicalPlan = {
    if (!GlutenConfig.get.enableRewriteSelfJoinInInPredicate) {
      plan
    } else {
      rewrite(plan)
    }
