...
class SessionCatalog(
  ...
  lazy val externalCatalogCache = new CatalogTableCache(
    columnarConf.omniColumnarCatalogCacheSize,
    columnarConf.omniColumnarCatalogCacheExpireTime)
  ...
  def invalidateCachedTable(key: QualifiedTableName): Unit = {
    ...
    externalCatalogCache.invalidateCachedTable(key)
    externalCatalogCache.invalidateCachedPartition(key.database, key.name)
  }
  ...
  def invalidateAllCachedTables(): Unit = {
    ...
    externalCatalogCache.invalidateAllCachedTables()
    externalCatalogCache.invalidateAllCachedPartition()
  }
  ...
  def lookupRelation(name: TableIdentifier): LogicalPlan = {
    synchronized {
      ...
      if (db == globalTempViewManager.database) {
        ...
        val cacheMetadata = externalCatalogCache.getCachedTable(db, table)
        if(cacheMetadata == null) {
          val metadata = externalCatalog.getTable(db, table)
          externalCatalogCache.cacheTable(db, table, metadata)
          getRelation(metadata)
        } else {
          getRelation(cacheMetadata)
        }
      } else {
  ...
  def refreshTable(name: TableIdentifier): Unit = synchronized {
    lookupTempView(name).map(_.refresh).getOrElse {
      ...
      externalCatalogCache.invalidateCachedTable(qualifiedTableName)
      externalCatalogCache.invalidateCachedPartition(dbName, tableName)
    }
  }
  ...
  def listPartitionsByFilter(
    ...
    val db = formatDatabaseName(tableName.database.getOrElse(getCurrentDatabase))
    val table = formatTableName(tableName.table)
    val cachedPartition = externalCatalogCache.getCachedPartitions(db, table, predicates, () => {
      requireDbExists(db)
      requireTableExists(TableIdentifier(table, Option(db)))
      val partitions = externalCatalog.listPartitionsByFilter(db, table, predicates, conf.sessionLocalTimeZone)
      partitions
    })
    cachedPartition
  }
