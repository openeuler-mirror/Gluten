...
case class OmniInsertIntoHadoopFsRelationCommand(
  ...
  override def run(sparkSession: SparkSession, child: SparkPlan): Seq[Row] = {
    ...
    SparkShimLoader.getSparkShims.checkColumnNameDuplication(
      outputColumnNames,
    ...
    val doInsertion = if (mode == SaveMode.Append) {
      ...
      (mode, pathExists) match {
        ...
        case (SaveMode.Ignore, exists) =>
          !exists
        case (s, exists) =>
          throw SparkShimLoader.getSparkShims.unsupportedSaveModeError(s, exists)
      }
    }
  ...
  override protected def withNewChildInternal(
                                               newChild: LogicalPlan): OmniInsertIntoHadoopFsRelationCommand = copy(query = newChild)
}
