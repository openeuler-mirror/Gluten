 ...
 */
case class OmniInsertIntoHiveTable(
                                table: CatalogTable,
                              ...
                              ) extends OmniSaveAsHiveFile with V1WriteCommand with V1WritesHiveUtils {
  ...
  override protected def withNewChildInternal(newChild: LogicalPlan): OmniInsertIntoHiveTable =
    copy(query = newChild)
}

object OmniInsertIntoHiveTable extends V1WritesHiveUtils {
  def apply(
             table: CatalogTable,
             partition: Map[String, Option[String]],
             query: LogicalPlan,
             overwrite: Boolean,
             ifPartitionNotExists: Boolean,
             outputColumnNames: Seq[String]): OmniInsertIntoHiveTable = {
    ...
    new OmniInsertIntoHiveTable(table, partition, query, overwrite, ifPartitionNotExists,
