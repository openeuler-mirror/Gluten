...
/**
 * Mutable row backed by [[OmniColumnVector]] columns; used for interpreted Omni projection (partial
 * project UDF materialization) without Arrow buffers.
 *
 * @since 2026-2026
 */
public final class OmniColumnarRow extends InternalRow {
    private int rowId;
    private final OmniColumnVector[] columns;

    /**
     * Create a mutable row backed by writable Omni columns.
     *
     * @param writableColumns columns to write
     */
    public OmniColumnarRow(OmniColumnVector[] writableColumns) {
        this.columns = writableColumns;
    }

    /**
     * Set the row id used by all getter and setter methods.
     *
        ...
        if (value == null) {
            ...
            if (dt instanceof BooleanType) {
                ...
                if (!(value instanceof BigDecimal)) {
                    throw new IllegalArgumentException("Decimal value should be BigDecimal: " + value);
                }
                ...
                if (!(value instanceof CalendarInterval)) {
                    throw new IllegalArgumentException("Interval value should be CalendarInterval: " + value);
                }
                setInterval(ordinal, (CalendarInterval) value);
            } else if (dt instanceof StringType) {
                if (!(value instanceof UTF8String)) {
                    throw new IllegalArgumentException("String value should be UTF8String: " + value);
                }
                setUTF8String(ordinal, (UTF8String) value);
            } else if (dt instanceof BinaryType) {
                if (!(value instanceof byte[])) {
                    throw new IllegalArgumentException("Binary value should be byte array: " + value);
                }
    ...
    @Override
    public void setBoolean(int ordinal, boolean isValue) {
        columns[ordinal].putBoolean(rowId, isValue);
    }
    ...
    /**
     * Set a UTF8 string value at the current row.
     *
    ...
    /**
     * Set a binary value at the current row.
     *
     * @param ordinal column ordinal
     * @param value value to write
     */
