...
/**
 * A class that reads Parquet files using JNI (Java Native Interface).
 * It initializes the JNI reader, processes filters, and handles columnar batch reading.
 */
public class ParquetColumnarBatchScanReader {
    ...
    private boolean nativeSupportDateRebase;
    ...
    /**
     * Constructor for ParquetColumnarBatchScanReader.
     *
     * @param requiredSchema     The schema of the data that is required.
     * @param datetimeRebaseSpec Specification for timestamp rebase.
     * @param int96RebaseSpec    Specification for INT96 rebase.
     * @param parquetTypes       List of parquet types.
     */
    public ParquetColumnarBatchScanReader(StructType requiredSchema, RebaseSpec datetimeRebaseSpec,
        ...
        this.dateRebaseFunc = datetimeRebaseSpec.mode() == null ? date -> date
    ...
    private void addJulianGregorianInfo(JSONObject job) {
        if (Arrays.stream(requiredSchema.fields()).noneMatch(field -> field.dataType() instanceof TimestampType)
                && Arrays.stream(requiredSchema.fields()).noneMatch(field -> field.dataType() instanceof DateType)) {
            return;
        }
    ...
    /**
     * Initializes the Parquet reader with the given path and capacity.
     *
     * @param path     The path to the Parquet file.
     * @param capacity The capacity for the reader.
     * @return The native pointer to the initialized reader.
    ...
    /**
     * Initializes the record reader with start, end offsets, field names, and a pushed filter.
     *
     * @param start        The start offset within the Parquet file.
     * @param end          The end offset within the Parquet file.
     * @param fieldNames   The field names to read.
     * @param pushedFilter The filter to push down.
     * @return The native pointer to the initialized record reader.
     * @throws UnsupportedEncodingException If the encoding is not supported.
     */
    ...
    /**
     * Retrieves all field names from the Parquet file.
     *
    ...
    private void tryToAdjustDateVec(IntVec intVec, long rowNumber, int index) {
        Type parquetType = parquetTypes.get(index);
        if (parquetType == null) {
            throw new RuntimeException("parquetType is null. index: " + index);
        }
        if (parquetType.getLogicalTypeAnnotation() instanceof LogicalTypeAnnotation.DateLogicalTypeAnnotation) {
            int adjustValue;
            for (int rowIndex = 0; rowIndex < rowNumber; rowIndex++) {
                adjustValue = (int) dateRebaseFunc.apply(intVec.get(rowIndex));
                intVec.set(rowIndex, adjustValue);
            }
        }
    }

    /**
     * Reads the next batch of records from the Parquet file.
     *
     * @param vecList       The list of vectors to store the read records.
     * @param missingColumns Array indicating which columns are missing.
     * @param types         The data types of the columns.
    ...
    public int next(Vec[] vecList, boolean[] missingColumns, List<DataType> types) {
        ...
        for (int i = 0; i < colsCount; i++) {
            ...
            if (type instanceof LongType) {
                ...
                vecList[i] = new IntVec(vecNativeIds[nativeGetId]);
                tryToAdjustDateVec((IntVec) vecList[i], rtn, i);
    ...
    /**
     * Closes the ParquetColumnarBatchScanReader by closing the underlying JNI reader and its associated parquetReader.
     */
    public void close() {
